...
class SolverBase:
  ...
  def prefix(self, problem):
    "Return file prefix for output files: root/problem.output_locations/problem/solver"
    root = self.options['results_dir']
    p = problem.__module__.split(".")[-1].lower()
    s = self.__module__.split(".")[-1].lower()
    return '/'.join([root, problem.output_location, p, s])
  ...
  def update(self, problem, t, u, p):
    ...
    if any(key for key in self.options.keys() if 'save' in key.lower()):
      result_dir = self.prefix(problem) 
    ...
    if self.options["save_solution"]:
      ...
      if (self._timestep - 1) % frequency == 0:
        ...
        if self._ufile is None:
          self._ufile = File(result_dir + "/refinement_level_" + str(refinement) + "_u.pvd")
        if self._pfile is None:
          self._pfile = File(result_dir + "/refinement_level_" + str(refinement) + "_p.pvd")
        self._ufile << u
        self._pfile << p
    ...
    if self.options["save_solution_at_t=T"]:
      if t >= problem.T:
        ...
        # Create files for saving
        if self._ufile is None:
          self._ufile = File(result_dir + "/refinement_level_" + str(refinement) + "_at_end" + "_u.pvd")
        if self._pfile is None:
          self._pfile = File(result_dir + self.prefix(problem) + "/refinement_level_" + str(refinement) + "_at_end" + "_p.pvd")
        self._ufile << u
        self._pfile << p
